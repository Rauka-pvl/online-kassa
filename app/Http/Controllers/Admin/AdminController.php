...
class AdminController extends Controller
{
    ...
    public function removeScheduleDay(Schedule $schedule, Request $request)
    {
        ...
        if ($scheduleDate) {
            // Если запись существует, помечаем её как неактивную (не удаляем, чтобы переопределить день недели)
            $scheduleDate->update(['is_active' => false]);
        } else {
            // Если нет конкретной даты, создаём запись с is_active = false
            // Это переопределит день недели только для этой конкретной даты
            $schedule->scheduleDates()->create([
                'date' => $date,
                'start_time' => null,
                'end_time' => null,
                'is_active' => false, // Отключаем только эту конкретную дату
            ]);
        }
        
        // Обновляем отношения, чтобы изменения сразу были видны
        $schedule->load('scheduleDates');
