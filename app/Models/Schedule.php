...
class Schedule extends Model
{
    ...
    public function isWorkingDate(string $date): bool
    {
        ...
        // Ограничение по датам графика (start_date / end_date)
        if ($this->start_date && $parsedDate->lt(Carbon::parse($this->start_date))) {
            return false;
        }

        if ($this->end_date && $parsedDate->gt(Carbon::parse($this->end_date))) {
            return false;
        }
        ...
        if ($this->relationLoaded('scheduleDates')) {
            $scheduleDate = $this->scheduleDates->first(function ($sd) use ($date) {
                return $sd->date->format('Y-m-d') === $date;
            });
        } else {
        ...
        // Если есть конкретная дата в schedule_dates
        if ($scheduleDate) {
            // Если is_active = false, день нерабочий
            if (!$scheduleDate->is_active) {
                return false;
            }
            // Если is_active = true, день рабочий
            return true;
        }
    ...
    public function getWorkingHoursForDate(string $date): ?array
    {
        ...
        // Если есть конкретная дата в schedule_dates
        if ($scheduleDate) {
            // Если is_active = false, возвращаем null (день нерабочий)
            if (!$scheduleDate->is_active) {
                return null;
            }
            // Если is_active = true и есть время, возвращаем рабочие часы
            if ($scheduleDate->start_time && $scheduleDate->end_time) {
                return [
                    'start' => $scheduleDate->start_time,
                    'end' => $scheduleDate->end_time
                ];
            }
            // Если is_active = true, но нет времени, используем день недели
        }

        // Если нет конкретной даты или она активна без времени, используем день недели
        $dayName = strtolower(Carbon::parse($date)->format('l'));
