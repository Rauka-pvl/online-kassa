                    ...
                    <div class="mb-4">
                        ...
                        <div id="schedule-dates-container">
                            @if($schedule->scheduleDates->count() > 0)
                                @foreach($schedule->scheduleDates as $index => $scheduleDate)
                                    <div class="schedule-date-item mb-3 p-3 border rounded">
                                        <div class="row align-items-end">
                                            <div class="col-md-3">
                                                <label class="form-label">Дата</label>
                                                <input type="date" class="form-control" name="schedule_dates[{{ $index }}][date]" value="{{ $scheduleDate->date->format('Y-m-d') }}">
                                            </div>
                                            <div class="col-md-3">
                                                <label class="form-label">Начало</label>
                                                <input type="time" class="form-control" name="schedule_dates[{{ $index }}][start_time]" value="{{ $scheduleDate->start_time ? \Carbon\Carbon::parse($scheduleDate->start_time)->format('H:i') : '09:00' }}">
                                            </div>
                                            <div class="col-md-3">
                                                <label class="form-label">Окончание</label>
                                                <input type="time" class="form-control" name="schedule_dates[{{ $index }}][end_time]" value="{{ $scheduleDate->end_time ? \Carbon\Carbon::parse($scheduleDate->end_time)->format('H:i') : '18:00' }}">
                                            </div>
                                            <div class="col-md-2">
                                                <div class="form-check">
                                                    <input class="form-check-input" type="checkbox" name="schedule_dates[{{ $index }}][is_active]" value="1" {{ $scheduleDate->is_active ? 'checked' : '' }}>
...
let scheduleDateIndex = {{ $schedule->scheduleDates->count() > 0 ? $schedule->scheduleDates->count() : 1 }};
